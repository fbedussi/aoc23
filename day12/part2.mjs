...
const input = lines
  ...
  .map(line => {
    ...
    return [{springs: replaceUnknowns(springs, unknowns, unknownsIndexes), unknowns, unknownsIndexes}, groups + ',' + groups+ ',' + groups+ ',' + groups+ ',' + groups]
  })
...
const match = ({springs}, groups) => {
  ...
  return springs.map(group => group.length).join(',') === groups
}
